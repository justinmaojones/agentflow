...
class DQN(BaseAgent, DiscreteActionAgentSource):
    ...
    def build_net(self, name):
        ...
        Q_model = tf.keras.Model(
            inputs=state_input, outputs=self.q_fn(state_input, name=f"{name}/Q")
        )
    ...
    def build_model(self):
        ...
        with tf.name_scope("DQN"):
            ...
            assert (
                Q_train.shape.as_list() == action_onehot.shape.as_list()
            ), "Q_train shape (%s) and action shape (%s) must match" % (
                str(Q_train.shape),
                str(action_onehot.shape),
            )
            ...
            if not (Q_eval.shape.as_list() == action_onehot.shape.as_list()):
                raise InvalidArgumentError(
                    "Q_eval shape (%s) and action shape (%s) must match"
                    % (str(Q_eval.shape), str(action_onehot.shape))
                )

            Q_action_eval = tf.reduce_sum(
                Q_train * action_onehot, axis=-1, keepdims=True
            )
            ...
            if self.double_q:
                ...
                Q_state2_target_action = value_at_argmax(
                    Q_state2_eval, Q_state2_target, axis=-1
                )
            else:
    ...
    def td_huber_loss(self, model_outputs, reward, gamma, done, mask):
        y = tf.stop_gradient(
            reward + gamma * (1 - done) * model_outputs["Q_state2_target_action"]
        )
