...
class BootstrappedDQN(BaseAgent, DiscreteActionAgentSource):
    ...
    def _validate_q_fn(self, Q):
        if not (Q.shape.as_list() == [None, self.num_actions, self.num_heads]):
            err_msg = (
                "expected q_fn to output tensor with shape [None, %d, %d], but instead got %s"
    ...
    def build_net(self, name):
        ...
        if self.random_prior:
            Q_prior_multihead = tf.stop_gradient(
                self.q_prior_fn(state_input, name=f"{name}/Q_prior")
            )
            self._validate_q_fn(Q_prior_multihead)
            Q_multihead = Q_multihead + self.prior_scale * Q_prior_multihead

        Q_model = tf.keras.Model(inputs=state_input, outputs=Q_multihead)
    ...
    def build_model(self):
        ...
        with tf.name_scope("BootstrappedDQN"):
            ...
            inputs = {
                ...
                "mask": tf.keras.Input(
                    shape=(self.num_heads,), dtype=tf.float32, name="mask"
                ),
            }
            self.inputs = inputs
            ...
            Q_eval_masked = weighted_avg(
                Q_eval_multihead, inputs["mask"][:, None, :], axis=-1
            )
            ...
            Q_state2_eval = weighted_avg(
                Q_state2_eval_multihead, inputs["mask"][:, None, :]
            )
            ...
            if self.double_q:
                ...
                Q_state2_target_action = tf.reduce_max(
                    Q_state2_target_multihead, axis=-2
                )
            ...
            self.policy_masked_model = tf.keras.Model(
                [inputs["state"], inputs["mask"]], policy_eval_masked
            )
            self.policy_logits_model = tf.keras.Model(inputs["state"], Q_eval)
            self.policy_logits_masked_model = tf.keras.Model(
                [inputs["state"], inputs["mask"]], Q_eval_masked
            )
