...
class ScopedIdleTimer(IdleTimer):
    ...
    def summary(self):
        ...
        for k in self._timed_scopes:
            output[self._get_scoped_key(k)] = (
                float(self._timed_scopes[k]) / self.duration
            )
...
def load_hdf5(filepath, load_keys=None):
    assert load_keys is None or isinstance(
        load_keys, list
    ), "load_keys must be None or a list"
...
def load_multiple_experiments(
    resultsdir, allow_load_partial=False, load_empty=False, load_keys=None
):
    """
    ...
    for sd in sorted(savedirs):
        if os.path.isdir(savedirs[sd]):
            try:
                log, config = load_experiment_results(
                    savedirs[sd], allow_load_partial, load_keys
                )
...
def stack_logs(logs):
    ...
    for k in keys:
        ...
        assert (
            len(set(map(type, logs_k))) == 1
        ), "log object myst be same type across list"
...
def smooth(x, k=10.0):
    return np.convolve(x, np.ones(int(k)) * 1.0 / k)
