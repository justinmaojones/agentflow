...
class PrevEpisodeLengthsEnv(EnvFlow):
    ...
    def step(self, action):
        ...
        self._prev_episode_lengths = (
            output["done"] * self._curr_episode_lengths
            + (1 - output["done"]) * self._prev_episode_lengths
        )
        self._curr_episode_lengths = (
            output["done"] * np.zeros_like(output["reward"])
            + (1 - output["done"]) * self._curr_episode_lengths
        )
        output["episode_length"] = self._prev_episode_lengths

        if self.log is not None:
            self.log.append(
                f"{self.__class__.__name__}/episode_length", self._prev_episode_lengths
            )
