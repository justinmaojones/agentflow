...
def run(**cfg):
    ...
    for t in range(T):
        ...
        bootstrap_mask_probs = (
            1 - cfg["bootstrap_mask_prob"],
            cfg["bootstrap_mask_prob"],
        )
        bootstrap_mask_shape = (len(state), cfg["bootstrap_num_heads"])
        bootstrap_mask = np.random.choice(
            2, size=bootstrap_mask_shape, p=bootstrap_mask_probs
        )
        ...
        log.append(
            "position", step_output["position"], summary_only=not cfg["log_everything"]
        )
        ...
        if t >= cfg["begin_learning_at_step"] and t % cfg["update_freq"] == 0:
            ...
            for i in range(cfg["n_update_steps"]):
                if cfg["buffer_type"] == "prioritized":
                    ...
                    log.append(
                        "max_importance_weight", sample["importance_weight"].max()
                    )
                else:
                ...
                if (
                    ...
                    and not cfg["prioritized_replay_simple"]
                ):
        ...
        if t % cfg["n_steps_per_eval"] == 0 and t > 0:
            log.append(
                "test_ep_returns", test_agent(test_env, agent), summary_only=False
            )
