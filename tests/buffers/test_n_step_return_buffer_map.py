...
class TestNStepReturnBuffer(unittest.TestCase):
    def test_all(self):
        ...
        for t in range(len(buf)):
            ...
            self.assertAlmostEqual(np.abs(R - returns[t]).max(), 0, places=4)
            self.assertEqual(np.abs(D - returns_dones[t]).max(), 0)
            self.assertAlmostEqual(np.abs(action[t] - buf_action[t]).max(), 0, places=4)
            self.assertAlmostEqual(np.abs(state[t] - buf_state[t]).max(), 0, places=4)
            self.assertAlmostEqual(
                np.abs(state2[t + NSTEPS - 1] - buf_state2[t]).max(), 0, places=4
            )
            priority = np.ones(B) if len(kwargs[t]) == 0 else kwargs[t]["priority"]
            self.assertAlmostEqual(
                np.abs(priority - buf_priority.get(t)).max(), 0, places=4
            )
