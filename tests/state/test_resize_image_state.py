...
class TestResizeImageState(unittest.TestCase):
    ...
    def test_update_3_channels(self):
        ...
        expected_flat[0] = cv2.resize(x[0], resized_shape, interpolation=cv2.INTER_AREA).ravel()
        expected_flat[1] = cv2.resize(x[1], resized_shape, interpolation=cv2.INTER_AREA).ravel()
