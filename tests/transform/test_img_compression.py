...
class TestEncoder(unittest.TestCase):
    def test_normal(self):
        ...
        # check encodings
        np.testing.assert_array_equal(x2["state"][0][: len(expected_0)], expected_0)
        np.testing.assert_array_equal(x2["state"][1][: len(expected_1)], expected_1)
    ...
    def test_reshape(self):
        ...
        for i in range(3):
            expected = cv2.imencode(
                ".png", np.array(x["state"][i].reshape(4, 30), dtype="uint8")
            )[1]
            np.testing.assert_array_equal(x2["state"][i][: len(expected)], expected)
            np.testing.assert_array_equal(
